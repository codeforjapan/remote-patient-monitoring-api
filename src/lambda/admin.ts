...
export namespace Admin {
  export const postAdminLogin: APIGatewayProxyHandler = async (event) => {
    try {
      ...
      const res = bodyData.refreshToken
        ? await admin.refreshToken(bodyData.refreshToken)
        : await admin.signIn(bodyData.username, bodyData.password);
      if (validator.isPatientAPI(event) && bodyData.username) {
        ...
        return {
          ...
          body: JSON.stringify({
            ...
            policy_accepted: (patient as Patient).policy_accepted,
          }),
        };
      } else {
        return {
