...
const createConfig = (stackResources: any) => ({
  ...
  cognito: {
    ...
    adminUserPoolWebClientId: getPhysicalId(
      stackResources,
      "RPMAdminAppClient"
    ),
    nurseUserPoolId: getPhysicalId(stackResources, "RPMNurseUserPool"),
    nurseUserPoolWebClientId: getPhysicalId(
      stackResources,
      "RPMNurseAppClient"
    ),
    patientUserPoolId: getPhysicalId(stackResources, "RPMPatientUserPool"),
    patientUserPoolWebClientId: getPhysicalId(
      stackResources,
      "RPMPatientAppClient"
    ),
    oauthDomain: `${getPhysicalId(
      stackResources,
      "AdminUserPoolDomain"
    )}.auth.${provider.region}.amazoncognito.com`,
  },
  apiGateway: {
...
const mergeConfigResources = (config: any) => {
  ...
  if (isFileExists(CONFIG_FILE)) {
    ...
    if (
      configs.constructor === Array &&
      configs.findIndex(
        (item: any) => item.apiGateway.stageName === config.apiGateway.stageName
      ) > -1
    ) {
      configs.splice(
        configs.findIndex(
          (item: any) =>
            item.apiGateway.stageName === config.apiGateway.stageName
        ),
        1,
        config
      );
...
const getPhysicalId = (stackResources: any, logicalId: string) => {
  return (
    stackResources.find((r: any) => r.LogicalResourceId === logicalId)
      .PhysicalResourceId || ""
  );
};
